...
var bundleCdn = window.KORONA_EMULATOR_BASE || "/emulator/";
var bundleCdnLatest = bundleCdn;
var biosCdn = bundleCdn + "bios/";
var infoJsonUrl = bundleCdn + "assets/info.json";
var standaloneDownloadUrl = bundleCdn + "index.html";
var relativeBase = bundleCdn;
var coreDir = "cores/";
...
var queries = window.KORONA_EMULATOR_CONFIG ? {core: window.KORONA_EMULATOR_CONFIG.core, rom: window.KORONA_EMULATOR_CONFIG.romUrl} : Object.fromEntries(window.location.search.substring(1).split("&").map(i => i.split("=")).map(i => i.map(i => i && decodeURIComponent(i))));
...
function grab(url, type, success, fail) {
	if (url.startsWith("https://gam.onl/user/")) {
		window.KORONA_EMULATOR_WISP(url).then(function(data) { if (success) success(type === "text" ? new TextDecoder().decode(data) : data); }, function() { if (fail) fail(0); });
		return;
	}
...
function readyRomFetch() {
	...
	var romFilename = queries.rom.split("/").slice(-1)[0].replace(/\.disk\d+$/i, ".chd");
...
function prepareBios() {
	var gameBiosUrl = window.KORONA_EMULATOR_CONFIG && window.KORONA_EMULATOR_CONFIG.biosUrl;
	if (gameBiosUrl) {
		window.KORONA_EMULATOR_WISP(gameBiosUrl).then(async function(data) {
			FS.createPath("/", baseFsSystemDir.substring(1), true, true);
			if (!gameBiosUrl.endsWith(".zip")) {
				FS.writeFile(baseFsSystemDir + gameBiosUrl.split("/").slice(-1)[0], new Uint8Array(data));
			} else {
				var entries = await new zip.ZipReader(new zip.Uint8ArrayReader(new Uint8Array(data))).getEntries();
				for (var entry of entries) {
					if (entry.directory) continue;
					var name = u8atoutf8(entry.rawFilename).replaceAll("\\", "/");
					if (!name || name.startsWith("/") || name.split("/").includes("..")) continue;
					var contents = await entry.getData(new zip.Uint8ArrayWriter());
					FS.createPath("/", (baseFsSystemDir + name).split("/").slice(1, -1).join("/"), true, true);
					FS.writeFile(baseFsSystemDir + name, contents);
					if (core == "a5200" && name.endsWith(".a52")) FS.writeFile(baseFsSystemDir + "5200.rom", contents);
					if (core == "opera" && name == "panafz10.bin") {
						FS.writeFile(baseFsSystemDir + "panafz10-norsa.bin", contents);
						FS.writeFile(baseFsSystemDir + "panafz10ja-anvil-kanji.bin", contents);
					}
				}
			}
			biosReady = true;
		}).catch(function(error) { log("BIOS fetch failed: " + error); biosReady = true; });
		return;
	}
	if (bioses[core]) {
